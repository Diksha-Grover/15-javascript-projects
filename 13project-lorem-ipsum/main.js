const text = [
  ...
  form.addEventListener("submit", function (e) {
    // when we click on the submit button it submits to the server by default
    // but we don't want that so we will remove the default behaviour 
    e.preventDefault();
  
    const value = parseInt(quantity.value);
    // value property in javascript Specifies the value of the attribute
    // parseInt method parses a value as a string and returns it in integer.
    
    // console.log(typeof value);
    
    // above statement is added to convert string into number
    const random = Math.floor(Math.random() * text.length);
    // Math.random() returns a random number between 0 (inclusive),  and 1 (exclusive) 
    // 0-1 but not 1
  
    if (isNaN(value) || value < 0 || value > 9) {
      // NaN is taken because what if an empty value is passed

      // NaN stands for Not a Number. It represents a value which is not a valid number.
      // isNaN() method returns true if a value is NaN
      // it means if user enters number less than 0 and greater than 9 then have a random index from text array
      // logical OR ( || ) operator (logical disjunction) for a set of operands is true if and 
      // only if one or more of its operands is true
      result.innerHTML = `<p class="result">${text[random]}</p>`;
      // innerHTML property sets or returns the HTML content of an element
    } else {
